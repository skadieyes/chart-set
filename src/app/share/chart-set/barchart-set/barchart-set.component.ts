...
@Component({
    selector: 'cst-barchart-set',
    templateUrl: 'barchart-set.component.html',
    styleUrls: ['barchart-set.component.css'],
    providers: [ScreenLengthService, ContactService, ChartService, ChartlistService]
})

export class BarChartSetComponent implements OnInit, AfterViewInit {
    @ViewChild(BarChartComponent) barchart: BarChartComponent;

    barchartset: BarChartSetModel;
    ...
    constructor(
        ...
        this.barchartset = new BarChartSetModel();
    ...
    Save() {
        if (this.flag === false) {
            const chartlist = new ChartlistItemModel('', this.barchartset.Title_Text, this.barchartset.Series_Type);
    ...
    select_data() { // 这个方法做三件事 请求数据并存到Data里 获取所有Key 放到arr中  并清空图表中的数据，清空两个下拉框
        this._http.getData(this.barchartset.Data_source).subscribe(data => {
            ...
            this.barchartset.Xaxis_Data = [1, 2, 3, 4, 5, 6, 7];
            this.barchartset.Series_Data = [0, 0, 0, 0, 0, 0, 0];
            this.barchartset.Data_valuename = '';
            this.barchartset.Data_name = '';
            this.chart_refresh();
        });
    }
